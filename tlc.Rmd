...
El teorema del límite central (TCL) es, posiblemente, uno de los teoremas fundamentales en la estadística y se usa ampliamente en cualquier campo en el que se quiera inferir algo o hacer predicciones a partir de los datos recopilados. Una primera versión (simple) fue introducida en el siglo XVIII, primero por De Moivre y luego, de una manera más refinada por Laplace, pero no fue hasta alrededor de 1935 cuando se publicó el teorema tal y como lo conocemos hoy. El objetivo de estas **notas con fines didácticos** es explicar, en términos generales lo que dice y, lo que es más importante, _lo que no dice_.

```{r fig-nocap-margin-first, fig.cap="Simulaciones de una distribución normal para diferentes valores de n", fig.margin=TRUE, fig.width=4, fig.height=4, cache=TRUE, echo=FALSE}
...
De manera informal, el teorema establece que si tomamos muestras aleatorias de una determinada distribución y luego las promediamos, el resultado (es decir, la media de la muestra) se parecerá a una distribución normal cuantas más muestras tomemos. De forma más precisa, si ${X_{1},...,X_{n},...}$ son variables aleatorias _independientes e idénticamente distribuidas (i.i.d)_ y $\overline{X}_{n}=\big(X_{1}+...+X_{n}\big)/n$ es la media muestral, su normalización (estandarización) $$\frac{\overline{X}_{n}-E(\overline{X}_{n})}{\sqrt{Var(\overline{X}_{n})}}$$ 
converge (en distribución) con una distribución normal estándar $N(0,1)$.

```{r fig-nocap-margin-second, fig.cap="Simulaciones de una distribución normal con media y varianzas diferentes", fig.margin=TRUE, fig.width=3.5, fig.height=3.5, cache=TRUE, echo=FALSE}
library(pdplot2)
pdplot2(seq(-5, 5, 0.01), mean=c(0,0,0), sd=c(1,1.5,2), type="normal", show.color=TRUE)
```

Otra forma de decir esto es que, a medida que $n$ aumenta, la "diferencia" entre la distribución de $\overline{X}_{n}$ y una distribución normal $N(E(\overline{X}_{n}),Var(\overline{X}_{n}))$ tiende a cero. Conviene apreciar que al ser las variables _i.i.d._, $E(\overline{X}_{n})$ es lo mismo que la esperanza de cualquiera de las $X_{i}$ y que la $Var(\overline{X}_{n})$ es la misma que $\frac{Var(X_{i})}{n}$. Mediante un cálculo simple, se puede comprobar que el teorema es igual si reemplazamos todas las instancias de $\overline{X}_{n}$ con la suma $S_{n}=X_{1}+X_{2}+\cdots X_{n}$.

El ejemplo más común que se suele ver del TLC en acción es cuando se considera una distribución binomial. Supongamos que lanzamos una moneda $n$ veces y contamos el _número de caras_ obtenidas; podemos pensar en esto como la suma de
$n$ variables aleatorias independientes, $X$, con una distribución de Bernoulli, $X_{i}\sim Be(1/2)$ (suponiendo que la moneda no está trucada, obviamente). Entonces, el TLC nos dice que _a medida que_ $n$ _aumenta podemos aproximar el número de caras_ $S_{n}=X_{1}+X_{2}+\cdots X_{n}$ a una distribución normal con media $n/2$ y desviación típica $\sqrt{n/4}$.
